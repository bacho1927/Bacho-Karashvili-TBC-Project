            ...
            <header className={`z-[3] w-full text-[#F4F4F4] p-4 font-semibold sticky top-0 bg-[#1A1E1F] ${scrolled ? ' opacity-90 ' : ''} transition duration-300`}>
                <div className="flex items-center justify-between max-w-[1000px]  m-auto   xsm:px-8 md:px-2">
                    ...
                    <div className='absolute right-14   z-[5]   w-16   lg:hidden transition duration-300' onClick={()=>setShowSidebar(Sidebar => !Sidebar)} >
                            <div className={`absolute top-4 right-0 w-1/2 h-2 bg-gray-500 rounded-md transition duration-300  ${showSidebar ? 'line1 ' : ''}`}></div>
                            <div className={`absolute w-full h-2 bg-gray-500 rounded-md  transition duration-300 ${showSidebar ? 'line2 ' : ''}`}></div>
                            <div className={`absolute bottom-2  w-1/2 h-2 bg-gray-500 rounded-md transition duration-300 ${showSidebar ? 'line3 ' : ''}`}></div>
                    </div>
                </div>
                <div className={`transition-all duration-300 fixed  w-[400px]  inset-0 right-0 left-auto ${!showSidebar ? 'opacity-0' :'opacity-1'}    lg:hidden z-[4] `} id='sidebar '>
